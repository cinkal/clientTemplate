...
var Browser=(function(){
	...
	*/
	...
	Browser.createElement=function(type){
		Browser.__init__();
		var element = Browser.document.__createElement(type);
		// if(type == "input")	{
		...
		// 	}
		// 	promptStyleDOM.innerText="input::-webkit-input-placeholder, textarea::-webkit-input-placeholder {"+"color:"+this._promptColor+";"+"font-size:18px;"+"}"+"input:-moz-placeholder, textarea:-moz-placeholder {"+"color:"+this._promptColor+";"+"font-size:18px;"+"}"+"input::-moz-placeholder, textarea::-moz-placeholder {"+"color:"+this._promptColor+";"+"font-size:18px;"+"}"+"input:-ms-input-placeholder, textarea:-ms-input-placeholder {"+"color:"+this._promptColor+";"+"font-size:18px;"+"}";
		// }
		return element;
	}
...
var NodeId = (function() {
	...
	__class(NodeId,'laya.display.NodeId');
	var __proto=NodeId.prototype;

	NodeId._getId=function(){
		++__id;
		if (__id >= Number.MAX_SAFE_INTEGER) {
			__id = 1;
		}
		return __id;
	}
...
var Node=(function(_super){
	...
	__proto.setTag=function(tag) {
		this._tag = tag;
	}

	__proto.getTag=function() {
		return this._tag;
	}
	...
	*/
	...
	__proto.getChildByTag=function(tag){
...
var Sprite=(function(_super){
	...
	function Sprite(){
		...
		*/
		...
		this._director=laya.Director.getInstance();
		this._actionManager=this._director.getActionManager();
	}
	...
	*/
	...
	__proto.setActionManager=function(manager){
		if(this._actionManager!=manager){
			this.stopAllActions();
			this._actionManager.destory();
			this._actionManager = manager;
		}
	}

	__proto.getActionManager=function(){
		return this._actionManager;
	}

	__proto.getSpriteId=function(){
		return this._spriteId;
	}

	__proto.runAction=function(action){
		if(action==null) throw new Error("Argument must be non-nil");
		this._actionManager.addAction(action, this, this._displayedInStage);
		return action;
	}

	__proto.stopAllActions=function(){
		this._actionManager.removeAllActionsFromTarget(this);
	}

	__proto.stopAction=function(action){
		this._actionManager.removeAction(action);
	}

	__proto.stopActionByTag=function(tag){
		if(tag==-1) throw new Error("Invalid tag");
		this._actionManager.removeActionByTag(tag, this);
	}

	__proto.stopAllActionsByTag=function(tag){
		if(tag==-1) throw new Error("Invalid tag");
		this._actionManager.removeAllActionsByTag(tag, this);
	}

	__proto.stopActionsByFlags=function(flags){
		if(flags>0){
			this._actionManager.removeActionsByFlags(flags, this);
		}
	}

	__proto.getActionByTag=function(tag){
		if(tag==-1) throw new Error("Invalid tag");
		return this._actionManager.getActionByTag(tag, this);
	}

	__proto.getNumberOfRunningActions=function(){
		return this._actionManager.getNumberOfRunningActionsInTarget(this);
	}

	__proto.getNumberOfRunningActionsByTag=function(tag){
		return this._actionManager.getNumberOfRunningActionsInTargetByTag(this, tag);
	}
...
var Text=(function(_super){
	...
	*/
	...
	Text.setTextLeftToRight=function(){
		var style;
		style=Browser.canvas.source.style;
		style.display="none";
		style.position="absolute";
		style.direction="ltr";
		Render._mainCanvas.source.style.direction="ltr";
		laya.display.Text.RightToLeft=false;
		Browser.document.body.appendChild(Browser.canvas.source);
	}
